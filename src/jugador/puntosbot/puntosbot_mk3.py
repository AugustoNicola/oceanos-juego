...
class PuntosBotMk3(JugadorBase):
	...
	def decidirAcciónDeRobo(self):
		...
		if self._verbose:
			...
			print(f"Valor descarte 0: {valorEstimadoDescarte0:.2f} ({self._juego.topeDelDescarte[0]})")
			print(f"Valor descarte 1: {valorEstimadoDescarte1:.2f} ({self._juego.topeDelDescarte[1]})")
	...
	def decidirAcciónDeDúos(self):
		...
		if self._juguéPeces:
			if not (
				self._listaDeEventos[-1].acción == Acción.Dúos.JUGAR_PECES and
				self._listaDeEventos[-1].jugador == self._númeroDeJugador and
				self._listaDeEventos[-1].parámetros["_cartaRobada"] != None
			):
				raise Exception("El evento de jugar dúo de peces está raro...")
			
			# Hemos robado una carta del mazo; podemos sacarla del mazo estimado
			cartaRobadaConPeces = self._listaDeEventos[-1].parámetros["_cartaRobada"]
		...
		if self._buscarDúoParaJugar(Carta.Tipo.BARCO) != None:
			...
			return (Acción.Dúos.JUGAR_PECES, self._buscarDúoParaJugar(Carta.Tipo.PEZ), None)
	...
	def _valorDeCarta(self, carta: Carta, explorar: bool = False) -> float:
		...
		if carta.esColeccionable():
			...
			if self._cantidadDeCartasDeTipoEnMano(carta.tipo) == 0:
				...
				#  más una bonificación por completar el dúo
				#  más el valor promedio que obtenemos de la carta robada
				valor += 1.0 + self._config.COMPLETAR_DÚO[carta.tipo]
				if explorar:
			...
			if self._cantidadDeCartasDeTipoEnMano(Carta.Tipo.TIBURÓN) == 0:
				...
				# Si podemos completar el dúo con esta carta, vale el punto del dúo
				#  más una bonificación por completar el dúo
				#  más el valor promedio de la mejor mano que vimos entrar a la mano de algún jugador
				valor += 1.0 + self._config.COMPLETAR_DÚO[carta.tipo]
				if explorar:
			...
			if cantidadDeSirenas == 3:
				...
				#  más una bonificación por conseguir una sirena
